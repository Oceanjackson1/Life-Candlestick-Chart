...
用户会提供他们的性别、出生年份、城市、家庭背景、大学及专业、工作地和岗位信息，以及个人特质。
你需要：
1. 复盘他们的过去：根据年龄和经历生成过去的 K 线波动。
2. 预测他们的未来轨迹：结合当前的信息和个人特质，在剩余的年龄里（直到 80 岁）合理推理并预测他们未来的人生走向（高潮、低谷、机遇或危机），体现在运势分数（0-100）上。
3. 特质会影响人生曲线的波动幅度和趋势方向。例如"冒险"型人格会有更大的起伏，"稳重"型人格曲线更平滑。"乐观"的人整体趋势偏上行，"敏感"的人波动会更频繁。
4. 为每个有故事的年龄生成一段简短的故事描述（story字段），描述这一年可能发生的事件或心境变化。大约每3-5年一个故事即可。
...
[
    {
      ...
      "milestone": string, // 如果这一年有重大事件，写在这里，否则留空字符串 ""
      "story": string, // 这一年的故事描述，没有则留空字符串 ""
      "isBull": boolean // 如果 close >= open 则为 true，否则为 false
    }
]

请确保生成严格连贯、带有戏剧性起伏的 81 年数据，并用真实感极强的 milestone 和 story 预测未来。`;
